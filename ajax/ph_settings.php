...
$currentImg = get_post_meta( $id, "ph_picture_url", true );

$currentImgId = get_post_meta( $id, "ph_picture_id", true );

$currentHeadline = get_post_meta( $id, "ph_headline", true );

$currentOrder = get_post_meta( $id, "ph_order", true );
?>
...
$query = $wpdb->prepare( "SELECT guid, ID FROM $wpdb->posts WHERE post_type='attachment' AND post_parent = %d", $id );
$attachments = $wpdb->get_results( $query );

$isImageFromPost = false;
foreach ( $attachments as $picture ):

	if ( $currentImgId == $picture->ID ) {
...
<script type="text/javascript" src="<?php echo $baseUrl; ?>js/ph_settings.min.js"></script>
